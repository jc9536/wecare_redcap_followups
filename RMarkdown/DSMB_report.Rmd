...
# Helper: robust date parser that tries multiple formats + Excel serials
parse_enroll_date <- function(x) {
  # 1) already Date/POSIX?
  if (inherits(x, "Date"))   return(x)
  if (inherits(x, "POSIXt")) return(as_date(x))

  # 2) character → try multiple orders
  if (is.character(x)) {
    out <- suppressWarnings(parse_date_time(
      x,
      orders = c(
        "Ymd", "Y-m-d", "mdY", "m/d/Y", "dmY", "d/m/Y",   # common
        "Ymd HMS", "Y-m-d H:M:S",                         # with time
        "b d Y", "d b Y"                                  # "Jan 2 2025"
      ),
      tz = "UTC",
      quiet = TRUE
    ))
    if (!all(is.na(out))) return(as_date(out))
  }

  # 3) numeric Excel serials (both 1900/1904 systems)
  if (is.numeric(x)) {
    # try 1900 system (Excel Windows)
    d1 <- as.Date("1899-12-30") + suppressWarnings(as.numeric(x))
    # try 1904 system (older Mac Excel)
    d2 <- as.Date("1904-01-01") + suppressWarnings(as.numeric(x))
    # pick the one that looks sane (>= 2019 & <= 2035, tweak as needed)
    ok1 <- !is.na(d1) & d1 >= as.Date("2019-01-01") & d1 <= as.Date("2035-12-31")
    ok2 <- !is.na(d2) & d2 >= as.Date("2019-01-01") & d2 <= as.Date("2035-12-31")
    if (any(ok1 | ok2)) return(ifelse(ok1, d1, d2))
  }
...
# Build the merged date string just like before, but do NOT filter after parsing
dat <- dat |>
  mutate(
    merged_date_chr = coalesce(
      as.character(ps_date),
    ...
    # Keep both the original raw and parsed versions
    date_enroll_raw = merged_date_chr,
    date_enroll     = parse_enroll_date(merged_date_chr),
    date_enroll_valid = !is.na(date_enroll)
  )

# DIAGNOSTIC: how many failed to parse?
cat("Enrollment date parsed for", sum(dat$date_enroll_valid), "of", nrow(dat), "rows\n")
if (any(!dat$date_enroll_valid)) {
  cat("Heads-up: some rows have unparseable dates — not dropping them.\n")
  # Show a small sample to help fix upstream mapping if needed
  print(
    dat |>
      filter(!date_enroll_valid) |>
      select(id_dyad, wecare_id_y, wecare_id_cg, site_id, treatment, date_enroll_raw) |>
      head(15)
  )
}

# --- Normalize treatment to 0/1 integers, keep original for reference ---
dat0 <- dat

# helper: coerce to integer safely (handles factor/character/"0"/"1"/blanks)
to_int01 <- function(x) {
  y <- suppressWarnings(as.integer(as.character(x)))
  # also handle "0"/"1" stored as character
  if (all(is.na(y)) && is.character(x)) {
    y <- ifelse(x %in% c("0","1"), as.integer(x), NA_integer_)
  }
  y
}
...
dat0 <- dat0 |>
  mutate(
    ...
    id_dyad       = coalesce(as.character(id_dyad),
                             substr(coalesce(as.character(wecare_id_y),
                                             as.character(wecare_id_cg),
                                             as.character(p_participant_id)), 1, 7))
  )

cat("=== RANDOMIZATION SNAPSHOT (as loaded) ===\n")
print(table(dat0$treatment, useNA = "ifany"))
cat("Total randomized (non-missing treatment):", sum(!is.na(dat0$treatment)), "\n\n")
```
...
r1 <- dat_locked |> filter(over_18 == 0, p_screen_legal_guardian == 0)
print(gt(r1 |> select(p_participant_id, site_id, id_dyad)) |> tab_header(title = "Reason 1: Under 18, no legal guardian"))
dat_locked <- anti_join(dat_locked, r1, by = names(dat_locked))
...
if (knitr::is_html_output()) {
  ...
  print(addmargins(table(dat_locked$merged_hear_more, dat_locked$site_id, useNA = "ifany")))
}

# Now apply filter
dat_locked <- dat_locked %>% dplyr::filter(merged_hear_more == 1)
...
if (knitr::is_html_output()) {
  ...
  print(addmargins(table(dat_locked$esc_label, dat_locked$site_id, useNA = "ifany")))
}

# Keep only complete screens (2) for downstream steps
dat_locked <- dat_locked %>% dplyr::filter(eligibility_screen_complete == 2L)

# Eligibility criteria again for Table 3.6
rv <- intersect(c("screen_race_1","screen_race_2","screen_race_3","screen_race_4"), names(dat_locked))
dat_locked <- dat_locked |>
  mutate(
...
print(gt(tabyl(dat_locked$merge_black, dat_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 3: youth not Black (Table 3.6)"))

print(gt(tabyl(dat_locked$screen_speak_read_english, dat_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 4: speak/read English (Table 3.6)"))
dat_locked <- dat_locked |> filter(screen_speak_read_english != 0)

print(gt(tabyl(dat_locked$screen_seek_therapy, dat_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 5: sought therapy past week (Table 3.6)"))
dat_locked <- dat_locked |> filter(screen_seek_therapy != 1)
...
elig2 <- intersect(c("eligible_visited_ed","eligible_hospitalized","eligible_taken_med",
                     ...
                     "eligible_anxious","eligible_depression",
                     "eligible_sch_close","eligible_sch_part",
                     "eligible_self_harm_y","eligible_sleep_problem_y"), names(dat_locked))
...
print(gt(tabyl(dat_locked$eligible_survey, dat_locked$site_id, show_na = TRUE)) |>
...
if (all(c("consent","site_id") %in% names(dat_locked))) {
  dat_locked <- dat_locked %>%
    mutate(
  ...
  if (knitr::is_html_output()) {
    ...
    print(addmargins(table(dat_locked$consent_lab, dat_locked$site_id, useNA = "ifany")))
  }
} else {
...
if (all(c("treatment","site_id") %in% names(dat_locked))) {
  tab_tx <- dat_locked %>%
    tabyl(treatment, site_id) %>%
    adorn_totals(c("row","col"))
  ...
  if (knitr::is_html_output()) {
    ...
    cat("\nTreatment by site (post-consent)\n")
    print(addmargins(table(dat_locked$treatment, dat_locked$site_id, useNA = "ifany")))
  }
} else {
...
# Baseline blocks (examples; mirrors STATA lists)
