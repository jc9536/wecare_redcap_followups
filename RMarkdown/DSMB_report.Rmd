...
# Counts (now aligned with CONSORT/QC logic)
consented_n       <- sum(s8$consent == 1, na.rm = TRUE)
did_not_consent_n <- sum(s8$consent == 0, na.rm = TRUE)

# Checks
gt::gt(
  s8 |> dplyr::filter(!is.na(treatment) & consent != 1) |>
    dplyr::select(p_participant_id, treatment, consent)
) |> gt::tab_header(title = "Check: Has treatment but consent != 1")

gt::gt(
  s8 |> dplyr::filter(is.na(treatment) & consent == 1) |>
    dplyr::select(p_participant_id)
) |> gt::tab_header(title = "Check: Consented but no treatment")

# CONSORT counters
did_not_complete_baseline_n <- sum(is.na(s8$treatment) & s8$consent == 1)
...
# Cache for Stage 9.5 reuse (optional but keeps everything in sync)
s8_counts <- list(
  consented_n = consented_n,
  did_not_consent_n = did_not_consent_n,
...
# Hand off for later chunks if you still use `dat`
dat <- s8
...
# A4: Eligibility survey
# Prefer to reuse the exact Section 7 outputs if they exist; otherwise, fall back.
if (exists("s7") && exists("s7_base") && exists("s7_counts")) {
  # Reuse exactly what Section 7 computed
  ...
  # Hand the completed survey frame (eligible + ineligible) forward as A4
  a4 <- s7

} else {
  # ---- fallback: derive A4 here (kept for robustness) ----------------------
...
if (exists("s8") && exists("s8_counts")) {
  a5 <- s8
  consented_n                <- s8_counts$consented_n
  did_not_consent_n          <- s8_counts$did_not_consent_n
  did_not_complete_baseline_n<- s8_counts$did_not_complete_baseline_n
  randomized_n               <- s8_counts$randomized_n
  treatment_n                <- s8_counts$treatment_n
  control_n                  <- s8_counts$control_n
} else {
...
{
  # coalesce helper so labels never print "NA"
  nz <- function(x) ifelse(is.na(x), 0L, x)

  # Build labels once (all values come from K)
  lbl <- list(
    ...
    m3_t_breakdown   = glue::glue("Active\nn = {nz(K$m3_t_active)}\nCompleted\nn = {nz(K$m3_t_completed)}\nMissed\nn = {nz(K$m3_t_missed)}\nExited\nn = {nz(K$m3_t_exited)}"),
    m3_c_breakdown   = glue::glue("Active\nn = {nz(K$m3_c_active)}\nCompleted\nn = {nz(K$m3_c_completed)}\nMissed\nn = {nz(K$m3_c_missed)}\nExited\nn = {nz(K$m3_c_exited)}"),

    m6_t             = glue::glue("6-Month Follow Up: Treatment\nn = {nz(K$m6_t_total)}"),
    m6_c             = glue::glue("6-Month Follow Up: Control\nn = {nz(K$m6_c_total)}"),

    m6_t_breakdown   = glue::glue("Active\nn = {nz(K$m6_t_active)}\nCompleted\nn = {nz(K$m6_t_completed)}\nMissed\nn = {nz(K$m6_t_missed)}\nExited\nn = {nz(K$m6_t_exited)}"),
    m6_c_breakdown   = glue::glue("Active\nn = {nz(K$m6_c_active)}\nCompleted\nn = {nz(K$m6_c_completed)}\nMissed\nn = {nz(K$m6_c_missed)}\nExited\nn = {nz(K$m6_c_exited)}")
  )

    # DOT with revised right-column positions (one row up) and extra right spacing
  dot <- glue::glue('
  digraph consort {{
    ...
    # right-side boxes
    hm_n     [label="{lbl$hear_more_n}"];     # to the right of TOP (moved up)
    dnwp_pre [label="{lbl$elg_screen_dnwp}"]; # to the right of hm_y (moved up)
    inel_scr [label="{lbl$elg_ineligible}"];  # to the right of elg_c (moved up)
    dnwp_svy [label="{lbl$elg_svy_dnwp}"];    # to the right of elg_e (moved up)
    inel_svy [label="{lbl$elg_svy_inelig}"];  # to the right of svy_t (moved up)
    ncons    [label="{lbl$did_not_consent}"]; # to the right of svy_e (moved up)
    nbase    [label="{lbl$did_not_baseline}"];# to the right of cons (moved up)

    # invisible gutter spacers to push right column further right
    ...
    # revised row alignment (right boxes one row up + gutter to push them right)
    ...
    # horizontal connectors (no vertical constraints)
