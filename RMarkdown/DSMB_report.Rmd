...
s8u <- s8 |>
  dplyr::arrange(p_participant_id, merged_date) |>
  dplyr::group_by(p_participant_id) |>
  dplyr::summarise(
...
dups <- s8 |>
  dplyr::count(p_participant_id, name = "rows") |>
  dplyr::filter(rows > 1)
...
if (exists("s8") && exists("s8_counts")) {
  ...
  # ---- Force row-level randomization counts (keep consent the way you had it) -
  # 'a5' is the Section 8 base (eligible_survey==1) with the consent flag.
  # We now define randomized, treatment, control as ROW counts.
  randomized_n <- sum(!is.na(a5$treatment))                 # row-level randomized
  treatment_n  <- sum(a5$treatment == 1, na.rm = TRUE)      # row-level by arm
  control_n    <- sum(a5$treatment == 0, na.rm = TRUE)      # row-level by arm
  # (Leave consented_n and did_not_complete_baseline_n exactly as before)
} else {
