...
DATA_LOCK <- as.Date("2025-12-01")  # used in some filters; update as needed
...
s8 <- s8_base |>
  dplyr::mutate(
    consent = dplyr::case_when(
      ...
      record_id %in% c("H-F0115","H-F0085", "H-F0553", "K-F0419", "H-F0538", "K-F0717") ~ 1L,  # verbal / form issues
      TRUE ~ 0L
    )
  )
...
```
# 8.5) Site-level enrollment table for Lorie 

```{r warning=FALSE, message=FALSE}
# 8.5) Site-level enrollment table for Lorie ----------------------------------

# First, inspect site_id values so you can confirm coding
raw |>
  count(site_id, sort = TRUE)

# Recode site_id to hospital labels
# Edit this case_when if your actual site_id coding is different
site_label_fn <- function(x) {
  case_when(
    x %in% c(1, "1", "H", "HH", "Harlem", "Harlem Hospital") ~ "Harlem Hospital",
    x %in% c(2, "2", "K", "KH", "Kings", "Kings Hospital") ~ "Kings Hospital",
    TRUE ~ as.character(x)
  )
}

# Approached = all unique youth in raw, by site
site_approached <- raw |>
  mutate(site = site_label_fn(site_id)) |>
  distinct(record_id, site) |>
  count(site, name = "Approached")

# Build person-level consent/randomization table with site attached
# s8 already contains the eligible+consent/randomization stage rows
site_person <- s8 |>
  mutate(
    site = site_label_fn(site_id),
    treatment = suppressWarnings(as.integer(readr::parse_number(as.character(treatment)))),
    cassy_pos = cassy_result == 1
  ) |>
  arrange(record_id, merged_date) |>
  group_by(record_id) |>
  summarise(
    site        = first(na.omit(site)),
    consented   = as.integer(any(consent == 1, na.rm = TRUE)),
    randomized  = as.integer(any(!is.na(treatment))),
    treatment   = dplyr::coalesce(first(na.omit(treatment)), NA_integer_),
    cassy_pos   = as.integer(any(cassy_pos & !is.na(treatment), na.rm = TRUE)),
    .groups = "drop"
  )

# Summarise by site
site_enrollment <- site_person |>
  group_by(site) |>
  summarise(
    Consented    = sum(consented == 1, na.rm = TRUE),
    Randomized   = sum(randomized == 1, na.rm = TRUE),
    Control      = sum(treatment == 0, na.rm = TRUE),
    Intervention = sum(treatment == 1, na.rm = TRUE),
    `CASSY+`     = sum(cassy_pos == 1, na.rm = TRUE),
    .groups = "drop"
  ) |>
  left_join(site_approached, by = "site") |>
  select(site, Approached, Consented, Randomized, Control, Intervention, `CASSY+`)

# Add total row
site_enrollment_total <- site_enrollment |>
  summarise(
    site         = "Total",
    Approached   = sum(Approached, na.rm = TRUE),
    Consented    = sum(Consented, na.rm = TRUE),
    Randomized   = sum(Randomized, na.rm = TRUE),
    Control      = sum(Control, na.rm = TRUE),
    Intervention = sum(Intervention, na.rm = TRUE),
    `CASSY+`     = sum(`CASSY+`, na.rm = TRUE)
  )

lorie_tbl <- bind_rows(site_enrollment, site_enrollment_total)

lorie_tbl |>
  gt::gt() |>
  gt::tab_header(title = "Enrollment by Site") |>
  gt::fmt_number(columns = c(Approached, Consented, Randomized, Control, Intervention, `CASSY+`), decimals = 0)

lorie_tbl
```
```{r}
for (i in seq_len(nrow(lorie_tbl))) {
  cat(lorie_tbl$site[i], ":\n", sep = "")
  cat("Approached: ", lorie_tbl$Approached[i], "\n", sep = "")
  cat("Consented: ", lorie_tbl$Consented[i], "\n", sep = "")
  cat("Randomized: ", lorie_tbl$Randomized[i], "\n", sep = "")
  cat("Control: ", lorie_tbl$Control[i], "\n", sep = "")
  cat("Intervention: ", lorie_tbl$Intervention[i], "\n", sep = "")
  cat("CASSY+: ", lorie_tbl$`CASSY+`[i], "\n\n", sep = "")
}
```
...
LOCK <- DATA_LOCK # not directly used here, but kept for consistency
