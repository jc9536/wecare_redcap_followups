...
dot_locked <- anti_join(dot_locked, r1, by = names(dot_locked))
...
if (knitr::is_html_output()) {
  ...
  print(addmargins(table(dot_locked$merged_hear_more, dot_locked$site_id, useNA = "ifany")))
}

# Now apply filter
dot_locked <- dot_locked %>% dplyr::filter(merged_hear_more == 1)
...
if (knitr::is_html_output()) {
  ...
  print(addmargins(table(dot_locked$esc_label, dot_locked$site_id, useNA = "ifany")))
}

# Keep only complete screens (2) for downstream steps
dot_locked <- dot_locked %>% dplyr::filter(eligibility_screen_complete == 2L)

# Eligibility criteria again for Table 3.6
rv <- intersect(c("screen_race_1","screen_race_2","screen_race_3","screen_race_4"), names(dot_locked))
dot_locked <- dot_locked |>
  mutate(
...
print(gt(tabyl(dot_locked$merge_black, dot_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 3: youth not Black (Table 3.6)"))

print(gt(tabyl(dot_locked$screen_speak_read_english, dot_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 4: speak/read English (Table 3.6)"))
dot_locked <- dot_locked |> filter(screen_speak_read_english != 0)

print(gt(tabyl(dot_locked$screen_seek_therapy, dot_locked$site_id, show_na = TRUE)) |>
        tab_header(title = "Reason 5: sought therapy past week (Table 3.6)"))
dot_locked <- dot_locked |> filter(screen_seek_therapy != 1)
...
elig2 <- intersect(c("eligible_visited_ed","eligible_hospitalized","eligible_taken_med",
                     ...
                     "eligible_anxious","eligible_depression",
                     "eligible_sch_close","eligible_sch_part",
                     "eligible_self_harm_y","eligible_sleep_problem_y"), names(dot_locked))
...
print(gt(tabyl(dot_locked$eligible_survey, dot_locked$site_id, show_na = TRUE)) |>
...
if (all(c("consent","site_id") %in% names(dot_locked))) {
  dot_locked <- dot_locked %>%
    mutate(
  ...
  if (knitr::is_html_output()) {
    ...
    print(addmargins(table(dot_locked$consent_lab, dot_locked$site_id, useNA = "ifany")))
  }
} else {
...
if (all(c("treatment","site_id") %in% names(dot_locked))) {
  tab_tx <- dot_locked %>%
    tabyl(treatment, site_id) %>%
    adorn_totals(c("row","col"))
  ...
  if (knitr::is_html_output()) {
    ...
    cat("\nTreatment by site (post-consent)\n")
    print(addmargins(table(dot_locked$treatment, dot_locked$site_id, useNA = "ifany")))
  }
} else {
