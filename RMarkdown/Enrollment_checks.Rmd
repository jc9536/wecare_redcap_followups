...
csv_path  <- "/Users/jaimiechin/Library/CloudStorage/Box-Box/WeCare (Michael Wu)/Data/cleaned/dat_merged.csv"
  #file.path(OUT_DIR, "dat_merged.csv") # => replace entire line with location of .csv file for analysis within quotation (e.g.: "desktop/dat_merged.csv")
...
AS_OF_DATE <- as.Date("2025-10-20")
TARGET_H   <- 345
TARGET_K   <- 630

# Weeks to include (any day inside the week is fine)
WEEKS_INPUT <- as.Date(c("2025-09-22","2025-09-29","2025-10-06","2025-10-17", "2025-10-20"))
...
```{r}
sum(!is.na(base_df$merged_date))
sum(!is.na(locked_df$merged_date))

base_df %>%
  select(p_participant_id, merged_date) %>%
  arrange(merged_date)
