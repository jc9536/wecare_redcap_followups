...
# Handles: epoch seconds/ms, Excel serials (tightened), ISO/MDY strings, and YYYYMMDD numerics.
to_date_smart <- function(x,
                          tz_epoch = "UTC",
                          tz_local = "America/New_York",
                          min_year = 2010,   # discard anything older than this as likely bad parse
                          max_year = 2100) {
  ...
  # helper to bound-check years
  clip_years <- function(d) {
    bad <- !is.na(d) & (as.integer(format(d, "%Y")) < min_year | as.integer(format(d, "%Y")) > max_year)
    d[bad] <- as.Date(NA)
    d
  }
  ...
  if (length(idx_num)) {
    ...
    # 8-digit YYYYMMDD (e.g., 20240819)
    is_yyyymmdd <- nchar_num == 8 & grepl("^(19|20)\\d{6}$", x_chr[idx_num])

    # Excel serials: only accept plausible modern range (>= 25569 = 1970-01-01) and
    # cap to ~60k (~2064). Also reject if it looks like YYYYMMDD (8 digits).
    is_excel <- !is_yyyymmdd & !is.na(num) & num >= 25569 & num <= 60000

    # Epoch: treat >1e12 as ms, >1e8 as seconds (covers modern dates)
    ...
    if (any(is_yyyymmdd)) {
      ...
      d_ymd <- clip_years(d_ymd)
      out[idx_num[is_yyyymmdd]] <- d_ymd
    }

    # Excel serials (modern only)
    if (any(is_excel)) {
      d_xl <- as.Date(num[is_excel], origin = "1899-12-30")
      d_xl <- clip_years(d_xl)
      out[idx_num[is_excel]] <- d_xl
    }
    ...
    if (any(is_epoch_ms)) {
      ...
      posix_local <- as.POSIXct(format(posix, tz = tz_local, usetz = TRUE), tz = tz_local)
      d_ep_ms <- as.Date(posix_local)
      d_ep_ms <- clip_years(d_ep_ms)
      out[idx_num[is_epoch_ms]] <- d_ep_ms
    }
    ...
    if (any(is_epoch_s)) {
      ...
      posix <- as.POSIXct(s, origin = "1970-01-01", tz = tz_epoch)
      posix_local <- as.POSIXct(format(posix, tz = tz_local, usetz = TRUE), tz = tz_local)
      d_ep_s <- as.Date(posix_local)
      d_ep_s <- clip_years(d_ep_s)
      out[idx_num[is_epoch_s]] <- d_ep_s
    }
  }

  # Non-numeric strings: try ISO then MDY (with and without times)
  ...
  if (length(idx_str)) {
    ...
    miss <- is.na(d1)
    if (any(miss)) {
      d2 <- suppressWarnings(lubridate::ymd_hm(x_s[miss],  quiet = TRUE)); d1[miss] <- d2; miss <- is.na(d1)
    }
    if (any(miss)) {
      d3 <- suppressWarnings(lubridate::ymd_hms(x_s[miss], quiet = TRUE)); d1[miss] <- d3; miss <- is.na(d1)
    }
    if (any(miss)) {
      d4 <- suppressWarnings(lubridate::mdy(x_s[miss],     quiet = TRUE)); d1[miss] <- d4; miss <- is.na(d1)
    }
    if (any(miss)) {
      d5 <- suppressWarnings(lubridate::mdy_hm(x_s[miss],  quiet = TRUE)); d1[miss] <- d5; miss <- is.na(d1)
    }
    if (any(miss)) {
      d6 <- suppressWarnings(lubridate::mdy_hms(x_s[miss], quiet = TRUE)); d1[miss] <- d6
    }
...
ppid_y  <- str_sub(as.character(raw$wecare_id_y), 1, 7)
ppid_cg <- str_sub(as.character(raw$wecare_id_cg), 1, 7)

# Merge date (youth→caregiver→screen→baseline→p_baseline), then drop blanks
base_df <- raw |>
  mutate(
    p_participant_id = if_else(is.na(ppid_y) | ppid_y == "", ppid_cg, ppid_y),
...
locked_df <- base_df |> filter(merged_date < DATA_LOCK)
```
...
count_outreach <- function(dd, var) {
  ...
  tmp <- dd %>%
    ...
    tidyr::pivot_wider(names_from = site_code, values_from = n, values_fill = 0)
...
build_all <- function(df,
  ...
  # ----- ENROLLMENT -----
  if (want_enroll) {
    approached     <- df %>% distinct(p_participant_id, site_code) %>% count_by_site()
  ...
  if (want_fu) {
    dat_fu <- df %>%
      mutate(
        ...
        follow_sch_min_3m = as_date_safe(follow_sch_min_3m),
        follow_sch_min_6m = as_date_safe(follow_sch_min_6m),
